...
##
## Function for pausing R execution during the demo
##
pause <- function() {
  invisible(readline())
}

printMeAndPause <- function (text, ...) {
...
printMe("Connecting to Twitter API")
...
# Print the text of the received tweets
printMe("Printing some of the tweets received...\n")
print(head(tweets.df$text, n = 4L))
printMeAndPause("")

# Pre-process and prepare training data for making the model
printMe("Pre-processing tweets...")
TDM = preprocessData(tweets.df)

freq_words = findFreqTerms(TDM,lowfreq= 80)

printMe("\nTerms to be used to train the model (based on frequency): \n")
print(freq_words)
printMeAndPause("")
TDM = TDM[freq_words,]
#inspect(TDM[1:10,])
...
printMe("Formula to train the model: ")
print(model_formula)
printMeAndPause("")
...
printMe("Will now print model summary...\n")
print(mymodel)

printMe("\nSince the model has now been trained, we test and update the model...")
printMeAndPause("")
